...
const OfflineBuyNutrition = (props) => {
    ...
    const setApi = async (fileName) => {
      console.log({
        ...
        setIsLoading(true)
       await fetch(Api + "package/pay_package_offline", {
          method: "POST",
          headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
    ...
    const setUploadApi = async (image) => {
      ...
      setIsLoading(true);
      await fetch(Api + "package/upload_receipt", {
        method: "POST",
        body: image,
      })
       .then(response => response.json())
    ...
    return (
      <View style={{ flex: 1, backgroundColor: "#fff" }}>
        ...
        <MyAlert visible={successAlert}
                 message={'بارگزاری رسید با موفقیت انجام شد'}
                 title={"ثبت اطلاعات"}
                 type={"success"}
                 buttonOnPress={() => {
                   setSuccessAlert();
